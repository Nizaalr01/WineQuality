...
clff = DecisionTreeClassifier(criterion='gini', random_state=42)

# Realizar validación cruzada con 5 divisiones (puedes ajustar el número)
scores = cross_val_score(clff, X_train, y_train, cv=5)
...
# Crear un modelo de árbol de decisión con criterio 'entropy'
clf = DecisionTreeClassifier(criterion='entropy', random_state=42)
clf.fit(X_train, y_train)
clf_pred = clf.predict(X_test)
clf_acc = accuracy_score(clf_pred, y_test)
print('precisión del test: {:.2f}%'.format(clf_acc*100))

"""Entropy sale 85.05% y gini 82.84%, entonces se mantiene entropy."""
...
"""El primer informe presenta los resultados de la validación cruzada, que muestra un rendimiento sólido en diferentes particiones de los datos, con una puntuación media de validación cruzada del 83.60%.

El segundo informe tiene una precisión ligeramente superior para la clase 1, lo que significa que es mejor para predecir esa clase con precisión. Sin embargo, su recall es un poco más bajo para la misma clase, lo que indica que podría perder algunos casos positivos.

El tercer informe muestra un recall ligeramente superior para la clase 1, lo que significa que es mejor para capturar todos los casos positivos, pero su precisión es un poco más baja.

Si se busca un equilibrio entre precisión y recall, podría optar por el segundo informe.
